...
export default defineComponent({
  ...
  setup() {
    ...
    const openStoreItemDetail = (productId: StoreItem["id"]) => {
      //在新建storeItem时不可点击切换其他storeItem
    ...
    const creatingStoreItem = ref(false);
    const prepareCreateStoreItem = () => {
      if (creatingStoreItem.value) return;
      ...
      creatingStoreItem.value = true;
      openStoreItemDetail(-1);
    };
