...
const props = withDefaults(defineProps<{ mark: number }>(), { mark: 10 });

const { mark } = toRefs(props);

const formattedMark = computed(() => Math.round(mark.value / 2));
const amount = 5;

const emit = defineEmits<{ (e: "update:mark", newVal: number): void }>();

const handleStarClick = (index: number) => {
  if (index + 1 === formattedMark.value) {
    emit("update:mark", 0);
    return;
  }
  emit("update:mark", (index + 1) * 2);
};
</script>
<template>
  <div class="flex items-center space-x-0.5">
    <svg
      v-for="(star, index) in amount"
      :key="index"
      width="24"
      height="24"
      viewBox="0 0 24 24"
      ...
      :class="
        index < formattedMark
          ? 'fill-zinc-900'
          : 'fill-zinc-900/0 hover:fill-zinc-900/10'
      "
